...
pub fn build_system_prompt(config: &Config) -> String {
    let mut profile = AgentProfile::new(&config.agent.name)
        .with_personality(&config.agent.personality);
...
pub fn build_agent_system_prompt(agent: &AgentProfile, config: &Config, all_agent_names: &[String]) -> String {
    ...
    if config.tools.web_fetch {
        tool_descriptions.push(
            "Web fetch: You can fetch and read the content of any web page URL."
        );
    }
    ...
    if config.tools.web_search {
        tool_descriptions.push(
            "Web search: You can search the web to find current information."
        );
    }
    ...
    {
        let peers: Vec<&String> = all_agent_names.iter()
            .filter(|n| !n.eq_ignore_ascii_case(&agent.name))
            .collect();
        if !peers.is_empty() {
            let peer_list = peers.iter().map(|n| n.as_str()).collect::<Vec<_>>().join(", ");
